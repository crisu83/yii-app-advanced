...
# install packages
echo mysql-server mysql-server/root_password select "kidmemo" | debconf-set-selections
echo mysql-server mysql-server/root_password_again select "kidmemo" | debconf-set-selections
sudo apt-get install -y mysql-server-5.5

# create database
mysql -uroot -pkidmemo -e "CREATE DATABASE kidmemo;"
